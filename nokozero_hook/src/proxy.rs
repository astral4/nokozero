...
extern "system" fn DirectInput8Create(
    ...
    let f: F = unsafe { transmute(get_proc(s!("DirectInput8Create"))) };
    unsafe { f(hinst, version, riid, out, outer) }
}

#[unsafe(no_mangle)]
extern "system" fn DllCanUnloadNow() -> i32 {
    type F = unsafe extern "system" fn() -> i32;
    let f: F = unsafe { transmute(get_proc(s!("DllCanUnloadNow"))) };
    unsafe { f() }
}
...
extern "system" fn DllGetClassObject(
    ...
    let f: F = unsafe { transmute(get_proc(s!("DllGetClassObject"))) };
    unsafe { f(rclsid, riid, ppv) }
}

#[unsafe(no_mangle)]
extern "system" fn DllRegisterServer() -> i32 {
    type F = unsafe extern "system" fn() -> i32;
    let f: F = unsafe { transmute(get_proc(s!("DllRegisterServer"))) };
    unsafe { f() }
}
...
extern "system" fn DllUnregisterServer() -> i32 {
    type F = unsafe extern "system" fn() -> i32;
    let f: F = unsafe { transmute(get_proc(s!("DllUnregisterServer"))) };
    unsafe { f() }
}
